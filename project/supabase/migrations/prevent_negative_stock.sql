-- Update the create_order_from_cart function to check for stock availability
...
BEGIN
  ...
  -- 2. Check for sufficient stock for ALL items
  SELECT string_agg(p.name, ', ')
  INTO v_insufficient_stock
  FROM cart_items c
  JOIN products p ON p.id = c.product_id
  WHERE c.user_id = p_user_id
  AND (p.stock < c.quantity OR p.stock IS NULL);

  IF v_insufficient_stock IS NOT NULL THEN
    RAISE EXCEPTION 'Insufficient stock for products: %', v_insufficient_stock;
  END IF;
  ...
  -- 5. Move items from cart to order_items
